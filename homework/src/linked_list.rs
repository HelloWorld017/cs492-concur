...
impl<T> IterMut<'_, T> {
    ...
    pub fn insert_next(&mut self, element: T) {
        let mut current_node = match unsafe { self.head.as_mut() } {
            None => return self.list.push_back(element),
            Some(current_node) => current_node
        };

        let mut prev_node = match unsafe { current_node.prev.as_mut() } {
            None => return self.list.push_front(element),
            Some(prev_node) => prev_node
        };

        let mut elem = Box::new(Node::new(element));
        elem.next = self.head;
        elem.prev = current_node.prev;
